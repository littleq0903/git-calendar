...
def get_output_from_command(commanad):
    parsed_command = shlex.split(command)
    out, err = subprocess.Popen(parsed_command, stdout=subprocess.PIPE)
    if err:
...
class GitCalendar(object):
    def __init__(self, commit_number_log):
        ...
        one_to_ten = [str(x) for x in range(1, 10)]
    ...
    def gen_rendered_matrix(self, end_date):
        ...
        string_list = [ map(stringify, row) for row in matrix]
        string_list = [ map(self.get_commits_by_date , row) for row in string_list]
        return string_list
